...
from spring.annotations import Autowired, DeleteMapping, GetMapping, PostMapping, RequestMapping, RestController
...
def _write_project(options: ProjectOptions) -> Path:
    ...
    if not web_enabled:
        ...
        import_end = application.find("from spring.annotations", import_start)
    ...
    if options.ai or "ai" in modules:
        files[project_dir / "docs" / "AI配置说明.md"] = (
            ...
            "在业务服务中按需导入 spring.ai；也可以配置本地 Ollama。\n"
        )
    if options.cloud or "cloud" in modules:
