"""
SpringBootAI LangChain 自动配置 - 读取 application.yml 的 spring.langchain.* 配置，
构建并注册 LangChain classic 全套能力 Bean 到 BeanRegistry。

完全镜像 spring.ai.autoconfig 的设计：
- LangChainProperties dataclass 类型化绑定 spring.langchain.* 子树
...
def bind_langchain_config(lc_config: Dict[str, Any]) -> LangChainProperties:
    """从 spring.langchain 子树绑定出类型化的 LangChainProperties。"""
...
def _build_langchain_model(props: LangChainProperties,
    ...
    """
    from spring.langchain.adapters import SpringChatModelToLangChain
    ...
    if provider == "auto" or not provider:
        ...
        if spring_chat_model is None:
            raise ValueError(
                ...
                "请先 configure_ai() 或设置 spring.langchain.default-llm 为具体 partner。")
        return SpringChatModelToLangChain(spring_chat_model).build()

    # partner 创建
    from spring.langchain.partners import PartnerProviderFactory
...
def _build_langchain_embeddings(props: LangChainProperties,
    ...
    from spring.langchain.adapters import SpringEmbeddingToLangChain
...
def configure_langchain(registry: Optional[BeanRegistry] = None,
    ...
    lc_config = config.get_prefix_config("spring.langchain") or {}
    props = bind_langchain_config(lc_config)

    if not props.enabled:
        logger.info("spring.langchain.enabled=false，跳过 LangChain 模块装配")
        return {}
