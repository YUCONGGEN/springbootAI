"""
LangChain 模块核心入口 - 对齐 spring.ai.core 的 ChatClient 编程范式。
...
class LangChainResponse:
    ...
    Attributes:
        ...
        content: 同 output（兼容 spring.ai 命名）
...
class LangChainCore:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            config: 配置字典（application.yml 的 spring.langchain.* 子树）
    ...
    def _ensure_langchain_embedding(embedding: Any) -> Any:
        ...
        if hasattr(embedding, "embed_query"):
            return embedding
        from spring.langchain.adapters import to_langchain_embeddings
...
class LangChainCoreBuilder:
    ...
    def with_config(self, config: Dict[str, Any]) -> "LangChainCoreBuilder":
        """设置配置字典（application.yml 的 spring.langchain.* 子树）。"""
        self._config = config
        return self
