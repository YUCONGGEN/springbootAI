...
from spring.annotations.core import Component, SpringAnnotation, get_spring_annotations
from spring.ai.annotations import Tool
...
def _resolve_manager(instance: Any, bean_name: str) -> Any:
    ...
    if manager is None:
        raise RuntimeError(
            ...
            "enable spring.mcp or inject _mcp_client_manager"
        )
    return manager
...
class MCPClient(Component):
    ...
    ``name`` must match an entry under ``spring.mcp.clients``. Methods annotated
