...
from spring.ai.tools import CompositeToolRegistry, ToolExecutionPolicy
from spring.config.config_loader import config_loader
from spring.context.registry import BeanRegistry
from spring.mcp.client import build_client_manager, require_mcp_sdk
from spring.mcp.config import MCPProperties, bind_mcp_config
...
def configure_mcp(
    ...
    *,
    ...
    No network connection or SDK import occurs while ``spring.mcp.enabled`` is
    ...
    if not props.enabled:
        logger.info("spring.mcp.enabled=false; skipping MCP auto-configuration")
        return {}
