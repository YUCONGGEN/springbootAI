...
- [七、常见问题](#七常见问题)
- [八、SpringPy AI 模块](#八springpy-ai-模块)
...
**配置读取（混合方式）**：`configure_ai()` 读取 `spring.ai.*` 子树后，用类型化 `AIProperties` dataclass 绑定（[autoconfig.py](file:///Users/yu/Desktop/springboot_cloud_python/springboot_cloud_python-master/spring/ai/autoconfig.py)）。优先级：**环境变量 > application.yml > dataclass 默认值**。环境变量通过两条路径生效：① config_loader 解析 yml 的 `${ENV:default}` 占位符；② dataclass 字段 `metadata["env"]` 声明的 env 名作为覆盖安全网（即使 yml 写死字面值也能被同名 env 覆盖）。字段类型注解驱动自动类型转换（`int`/`float`/`bool`），无需手动 `int()`/`float()`。
...
**AI 可选依赖**：AI 模块采用"可选依赖 + 降级"设计，未安装任何 AI 依赖时仍可用（降级原生 HTTP + FakeChatModel）。安装 `requirements-ai.txt` 后自动启用 LangChain 生态适配（OpenAI/Ollama）。
...
assert props.openai.chat.temperature == 0.3 and isinstance(props.openai.chat.temperature, float)
...
**含义**：标注一个服务类使用 AI 客户端，框架按 provider 配置自动注入 ChatClient。
...
**含义**：将函数注册为可被 LLM 调用的工具（Function Calling），框架从签名自动生成 schema。
...
### 8.3 ChatClient 链式 API（对齐 Spring AI）
...
> 闭环 Function Calling（tools 自动注入请求体 + tool_call 循环执行回填续写）见 [8.9.1](#891-闭环-function-calling)。
...
| spring/ai/autoconfig.py | AIProperties 类型化绑定 + spring.ai.* 配置装配 Bean（ChatModel/EmbeddingModel/VectorStore/Memory/ChatClient） |

### 8.9 企业级能力

AI 模块在基础抽象之上补齐了 5 项企业级能力，复用框架已有的 AOP/重试/Prometheus 基础设施，开箱即用。

#### 8.9.1 闭环 Function Calling
...
#### 8.9.2 韧性：重试 + 熔断（复用 @Retryable/@CircuitBreaker）
...
#### 8.9.4 Prometheus 观测（复用框架 prometheus 配置）
...
#### 8.9.5 RedisVectorStore（RAG 持久化）

`RedisVectorStore` 用 Redis hash 持久化文档（键 `springpy:ai:vectorstore:{collection}`），支持多副本跨实例检索；注入 EmbeddingModel 实现检索时自动嵌入。配置 `vector-store.type: redis` 并传入 `redis_client` 即用，无 client 时安全降级为内存。
