...
当前版本是 `2.3.6`；支持 Python 3.10、3.11 和 3.12，许可证为 MIT。项目仍标记为 Beta。用于公网高并发、合规敏感或支付/订单/库存等核心系统前，必须完成目标数据库、流量模型、故障恢复和安全基线验证。内嵌 Gateway 适合内部路由，不替代公网 Nginx/Kong/WAF；Seata `distributed` 对接官方 TC + TCC 回调；`at` 模式通过 ORM 拦截器自动生成 undo_log 实现自动回滚。
...
| ✅ 企业级注解驱动 | [ENTERPRISE_ANNOTATIONS.md](https://github.com/YUCONGGEN/springbootAI/blob/master/doc/ENTERPRISE_ANNOTATIONS.md) | 随核心包 | `@EnableOAuth2`/`@EnableCsrf`/`@EnableDevTools`/`@EnableBatchProcessing` 等10个注解 |
| ✅ 数据库迁移 | [MIGRATION_MODULE.md](https://github.com/YUCONGGEN/springbootAI/blob/master/doc/MIGRATION_MODULE.md) | 随核心包 | Flyway风格版本化迁移/Undo回滚/迁移锁/变量替换 |
| ✅ Starter机制 | [STARTER_MODULE.md](https://github.com/YUCONGGEN/springbootAI/blob/master/doc/STARTER_MODULE.md) | 随核心包 | `pip install springbootAI[web/cloud/all]` 组合依赖 |
| ✅ CLI与脚手架 | [CLI_MODULE.md](https://github.com/YUCONGGEN/springbootAI/blob/master/doc/CLI_MODULE.md) | 随核心包 | `springbootai init/run/docs` 命令行工具 |
...
| 组件 | 当前版本 |
|------|----------|
| `springbootai` 框架 API | 2.3.6 |
| `springbootai.orm.pymybatis` | 2.3.6 |
| `springbootai.ai` AI 模块 | 2.3.6 |
| `springbootai.langchain` LangChain 模块 | 2.3.6 |
| Python | 3.10+ |
