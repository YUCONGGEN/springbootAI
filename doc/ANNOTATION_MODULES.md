...
**注解模块 = 常用工具注解集——就像瑞士军刀，每个注解解决一个特定问题。** 本文介绍五组最常用的注解工具：数据校验、条件装配、缓存、CSV 读写、并发保护。你不需要一次性全部学会——用到哪个看哪个就行。需要统一拦截 Service、返回后鉴权或重试兜底时，直接阅读 [AOP / 后置鉴权 / 重试恢复指南](AOP_SECURITY_RETRY.md)。
