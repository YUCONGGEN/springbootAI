# SpringBootAI CSV 模块使用文档

> 版本：SpringBootAI CSV 1.0.0 ｜ 框架版本：SpringBootAI 1.7.0
> 对齐 alibaba EasyExcel / commons-csv 的注解驱动 CSV 读写，**无可选依赖**（Python 标准库 `csv`），`pip install springbootAI` 即可用。
...
- **无可选依赖**：CSV 使用 Python 标准库 `csv`，无需 `pip install springbootAI[excel]`；注解声明与读写均开箱即用。
