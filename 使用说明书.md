# SpringPy 1.3.0 使用说明书

本文说明 SpringPy 应用的安装、启动、Web 开发、依赖注入、内嵌 PyMyBatis、事务、安全、部署和排错方式。内嵌 ORM 章节与 PyMyBatis 1.3.0 的核心行为保持一致。Java 项目迁移请同时阅读 `docs/JAVA_TO_PYTHON_MIGRATION.md`。
...
- Nacos、RabbitMQ、Seata、SkyWalking、Prometheus 依赖外部服务，不应只验证 import；Nacos 2.2+ Docker 还必须设置认证 token 和 identity。
...
| `@RefreshScope` | 配置变化时刷新 Bean | 有代理函数但未接入类创建链路；当前不要依赖自动刷新 |
| `@EnableFeignClients` | 扫描 Feign 接口 | 仅元数据；不会自动扫描注册 |
| `@FeignClient` | 创建远程 HTTP 客户端代理 | 仅元数据；需显式使用 `spring.cloud.feign` 中的客户端工厂 |
| `@SentinelResource` | 限流、业务异常 fallback | 受管 Bean 方法会包装；本实现不是 Alibaba Sentinel 客户端，必须按本地实现验证 |
| `@EnableGateway` | 启用 API Gateway | 仅元数据；不会创建路由或过滤器 |
| `@LoadBalanced` | 给 `@Bean` 工厂返回对象添加负载均衡标记 | `@Bean` 方法上会执行，但实际请求仍需配合框架负载均衡客户端 |
| `@GlobalTransactional` | 通过 Seata 管理全局事务 | 受管 Bean 方法会调用 Seata manager；需真实 Seata 环境验证，不支持嵌套 |
...
运行时建议 `block_ddl: true`。建表和变更表结构应由 Alembic、Flyway 或独立迁移脚本完成。不要依赖应用启动时自动建表。
...
配置 `rabbitmq.enabled: true` 并提供 host、port、username、password、virtual_host。开发环境也应在真实 Nacos、RabbitMQ、Seata 等环境执行集成、断线和重复投递测试。
...
| 端点 | 说明 |
|------|------|
| `/actuator/health` | Redis、数据库、Nacos、RabbitMQ、Seata 聚合状态 |
