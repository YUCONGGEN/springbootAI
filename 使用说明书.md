# SpringPy 1.6.0 使用说明书

本文说明 SpringPy 应用的安装、启动、Web 开发、依赖注入、内嵌 PyMyBatis、事务、安全、部署和排错方式。内嵌 ORM 章节与 PyMyBatis 1.4.0 的核心行为保持一致。v1.6.0 已内嵌 Sentinel 限流熔断、OpenTelemetry 分布式追踪、Seata HTTP-AT 分布式事务、API Gateway 网关、ORM DDL 自动建表，并新增 SpringPy AI 模块（对齐 Spring AI 2.0，ChatClient/Advisor/Tools/RAG/Function Calling/Redis 向量存储），无需部署外部 Server。Java 项目迁移请同时阅读 `docs/JAVA_TO_PYTHON_MIGRATION.md`。
