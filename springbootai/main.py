...
from springbootai.utils.banner import BannerPrinter
from springbootai.utils.logger import SpringLogger
from springbootai.logging.loguru_logger import init_logging, LoggingConfigError
from springbootai.orm.mybatis_integration import init_mybatis
from springbootai.annotations.cloud import EnableDiscoveryClient
from springbootai.annotations.security import EnableOAuth2, EnableCsrf
from springbootai.annotations.enterprise import (
    EnableDevTools,
    EnableConfigServer,
    EnableBus,
    EnableDataRest,
...
class SpringApplication:
    ...
    def _init_enterprise_components(self) -> None:
        ...
        try:
            from springbootai.security.secret_manager import resolve_secret_config
        ...
        if redis_config.get('enabled', True):
            try:
                from springbootai.utils.redis_client import init_redis
        ...
        try:
            from springbootai.security.jwt_utils import init_jwt
        ...
        if database_config.get('enabled', False) and orm_mode in {'sqlalchemy', 'both'}:
            try:
                from springbootai.orm.database import init_database
        ...
        if discovery_enabled:
            try:
                from springbootai.cloud.discovery import init_discovery
        ...
        if seata_config.get('enabled', False):
            try:
                from springbootai.cloud.seata import init_seata
        ...
        if config_center_enabled:
            try:
                ...
                if config_center_annotation:
                    ...
                    from springbootai.cloud.config_center import init_config_center
        ...
        if rabbitmq_config.get('enabled', False):
            try:
                from springbootai.messaging.rabbitmq import init_rabbitmq
        ...
        if kafka_config.get('enabled', False):
            try:
                from springbootai.messaging.kafka import init_kafka
                init_kafka(config)
        ...
        if prometheus_config.get('enabled', False):
            try:
                from springbootai.monitoring.prometheus import init_prometheus
    ...
    def _bootstrap_logging_configuration(self) -> ConfigLoader:
        """先静默加载完整配置，并在任何框架日志之前应用 logging 配置。

        ``ApplicationContext`` 会在构造时创建 ``ConfigLoader``。旧流程在此之前
        已输出了启动 INFO，导致 ``logging.level: WARNING`` 无法控制第一批日志。
        这里复用同一个配置加载器传给上下文，既支持 profile、.env 和环境变量覆盖，
        又避免二次读取配置及重复的“Loaded config”日志。
        """
        try:
            main_class_file = inspect.getfile(self.main_class)
        except (TypeError, OSError):
            main_class_file = os.path.join(os.getcwd(), "__main__.py")
        main_class_dir = os.path.dirname(os.path.abspath(main_class_file))
        config_loader = ConfigLoader(base_path=main_class_dir, log_events=False)
        config = config_loader.get_config()
        init_logging(self._section(config.get("logging")))
        return config_loader

    def _prepare_context(self) -> None:
        config_loader = self._bootstrap_logging_configuration()
        self.logger.info("Preparing application context...")
        self.application_context = ApplicationContext(
            self.main_class, config_loader=config_loader
        )
        try:
            ...
            config_loader.start_nacos_config_refresh(
                self.application_context.refresh_configuration
            )
    ...
    def _on_app_startup(self) -> None:
        ...
        self._register_admin_client(config, fail_fast)
        self._background_started = True

    def _register_admin_client(self, config: dict, fail_fast: bool) -> None:
        from springbootai.monitoring.admin_client import AdminClientProperties, SpringBootAdminClient
        properties = AdminClientProperties.from_config(config)
        if not properties.enabled:
            return
        try:
            self._admin_client = SpringBootAdminClient(properties)
    ...
    def _deregister_admin_client(self) -> None:
        if self._admin_client is not None:
            try:
    ...
    def _configure_web_lifecycle(self) -> None:
        ...
        app.router.add_event_handler('shutdown', self._deregister_admin_client)
    ...
    def _start_web_server(self, **kwargs) -> None:
        ...
        try:
            from springbootai.web.csrf import get_csrf_token_manager, CSRFMiddleware
...
def run_cli():
    ...
    if len(sys.argv) > 1 and sys.argv[1] in _KNOWN_SUBCOMMANDS:
        # 子命令模式：委托给 springbootai.cli.main
        from springbootai.cli.main import main as cli_main
        return cli_main()
