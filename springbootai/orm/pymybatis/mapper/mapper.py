...
class MapperProxy:
    ...
    def _result_type_from_return_annotation(self, method) -> Optional[Type]:
        ...
        if origin in _UNION_ORIGINS:
            ...
            # ``Mapping[str, Any] | None`` describes an already materialized
            # row dictionary.  It is not a model class that MapperProxy can
            # construct, so preserve the database row unchanged.
            if get_origin(candidate) in {dict, Mapping} or candidate in {dict, Mapping}:
                return None
            return candidate if isinstance(candidate, type) else None
        if origin is Mapping or annotation is Mapping:
            return None
