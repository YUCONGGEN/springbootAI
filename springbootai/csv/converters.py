...
``springbootai.excel.converters`` 的 ``Converter`` 接口与内置转换器（int/float/bool/str/
...
# 复用 Excel 转换器（springbootai.excel.converters 不依赖 openpyxl，可安全导入）
from springbootai.excel.converters import (
    Converter,
    StringConverter,
    IntegerConverter,
    FloatConverter,
...
def resolve_csv_converter(
    ...
    """CSV 转换器解析入口（委托 ``springbootai.excel.converters.resolve_converter``）。
