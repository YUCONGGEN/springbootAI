...
def _get_framework_version() -> str:
    ...
    try:
        import springbootai
        return springbootai.__version__
    except Exception:
        return 'unknown'
...
def _list_annotations() -> None:
    """列出可用注解。"""
    try:
        from springbootai.annotations import __all__ as annotations_all
...
def create_parser() -> argparse.ArgumentParser:
    ...
    # Keep the unified command in sync with ``springbootai.cli.scaffold`` (8000).
