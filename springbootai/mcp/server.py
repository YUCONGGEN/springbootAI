...
from springbootai.mcp.client import require_mcp_sdk
from springbootai.mcp.config import MCPConfigurationError, MCPServerProperties
...
class MCPServerAdapter:
    """Expose selected ``springbootai.ai.ToolRegistry`` entries through MCP.
    ...
    def _register_allowlisted_tools(self) -> None:
        if self.tool_registry is None or not hasattr(self.tool_registry, "names"):
            raise MCPConfigurationError("MCP server requires a springbootai.ai ToolRegistry")
