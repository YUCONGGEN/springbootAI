...
def bind_mcp_config(data: Mapping[str, Any] | None = None) -> MCPProperties:
    """Bind the ``springbootai.mcp`` mapping into validated properties."""
    data = data or {}
    ...
    if isinstance(raw_clients, Mapping):
        ...
        raise MCPConfigurationError("springbootai.mcp.clients must be an object or list")

    raw_server = _get(data, "server", {}) or {}
    if not isinstance(raw_server, Mapping):
        raise MCPConfigurationError("springbootai.mcp.server must be an object")
